            ...
            <nav className="w-4/5 h-full">
                <ul className="w-full h-full flex items-center justify-around">
                    {ListLinks.map((link, index) => (
                        <Link smooth to={`#${link}`} key={index} className="w-1/4 h-11/12 
                        border-r-2 border-gray-700 flex items-center justify-center text-xl 
                        tracking-normal cursor-pointer hover:text-blue-600 
                        hover:underline hover:scale-95 transition-transform
                        duration-300 italic outline-none">
                            {link}
                        </Link>
                    ))}
                </ul>
